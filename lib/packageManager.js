...
function installPackagesWithNpm(packages, existingPackages = {}) {
  ...
  for (const [pkg, version] of Object.entries(packages)) {
    ...
    if (existingPackages[pkg] && 
      ...
      // Use the first valid installation path that exists
      for (const sourcePath of existingPackages[pkg].versions[version].installPaths) {
      ...
      // If we found a valid path, continue to next package
      if (packagesToSymlink[pkg]) {
        continue;
      }
    }
